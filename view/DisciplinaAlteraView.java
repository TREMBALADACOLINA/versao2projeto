...
public class DisciplinaAlteraView extends JDialog {
	 ...
	 */
	public DisciplinaAlteraView(int posicaoArray, Disciplina disciplina) {
		setDefaultCloseOperation(JDialog.DO_NOTHING_ON_CLOSE);
		this.posArray = posicaoArray;
		this.disciplina = disciplina;
		setModal(true);
		setIconImage(Toolkit.getDefaultToolkit()
				.getImage(DisciplinaAlteraView.class.getResource("/br/com/ufpb/poo/projeto/images/ufpb.png")));
		...
		// setar jTxtField com os valores da disciplina
		textFieldNome.setText(this.disciplina.getNome());
		textFieldCodigo.setText(this.disciplina.getCodigo());
		textFieldArea.setText(this.disciplina.getArea());
		...
		JLabel lblCadastroDeDisciplinas = new JLabel("Alterar disciplina		");
	...
	private void altera() {
		Disciplina disciplina = new Disciplina(textFieldCodigo.getText().trim().toUpperCase(),
				textFieldNome.getText().trim().toUpperCase(), textFieldArea.getText().trim().toUpperCase());
			// validar campos
		if (disciplina.getNome().isEmpty()) {
			JOptionPane.showMessageDialog(null, "Por favor, digite o nome da disciplina!");
			...
			JOptionPane.showMessageDialog(null, "Por favor, digite a área da disciplina!");
			textFieldCodigo.requestFocus(); // Seta o foco para o componente
		} else if (gerente.verificaSeExisteDisciplina(this.posArray, disciplina)) {
			String mensagem = "Já existe disciplinas com esses dados!\n";
			// buscar a disciplina para ter uma referencia
			try {
				Disciplina d2 = gerente.buscaDisciplina(disciplina);
				mensagem = mensagem + "Disciplina: " + d2.getNome() + " - Cod: " + d2.getCodigo();
			} catch (Exception e1) {
			...
			gerente.editarDisciplina(this.disciplina.getCodigo(), disciplina);
			JOptionPane.showMessageDialog(this, "Disciplina Cadastrado com sucesso!");
			this.dispose();
		}
