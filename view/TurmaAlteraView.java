...
public class TurmaAlteraView extends JDialog {
	 ...
	 */
	public TurmaAlteraView(int posicaoArray, Turma turma) {
		setDefaultCloseOperation(JDialog.DO_NOTHING_ON_CLOSE);
		this.posArray = posicaoArray;
		this.turma = turma;
		setIconImage(Toolkit.getDefaultToolkit()
				.getImage(TurmaAlteraView.class.getResource("/br/com/ufpb/poo/projeto/images/ufpb.png")));
		...
		JLabel lblCadastroDeProfessores = new JLabel("Alterar turmas");
		...
		// setar jTxtField com os valores da turma
		textFieldMatriculaProfessor.setText(this.turma.getProfessor().getMatricula());
		textFieldCodigoDisciplina.setText(this.turma.getDisciplina().getCodigo());
		textFieldIdentificacaoDisciplina.setText(this.turma.getCodigo());
	...
	private void altera() {
		...
		if (textFieldMatriculaProfessor.getText().isEmpty()) {
			JOptionPane.showMessageDialog(null, "Por favor, digite a matricula do professor!");
			textFieldMatriculaProfessor.requestFocus(); // Seta o foco para o componente
		} else if (textFieldCodigoDisciplina.getText().isEmpty()) {
			JOptionPane.showMessageDialog(null, "Por favor, digite o código da disciplina!");
			...
			JOptionPane.showMessageDialog(null, "Por favor, digite o identificação da disciplina!");
			textFieldIdentificacaoDisciplina.requestFocus(); // Seta o foco para o componente
		} else if (gerente.verificaSeExisteProfessor(textFieldMatriculaProfessor.getText().trim().toUpperCase())) {
			JOptionPane.showMessageDialog(null, "Matrícula do professor inexistente!");
			textFieldMatriculaProfessor.requestFocus(); // Seta o foco para o componente
		} else if (gerente.verificaSeExisteDisciplina(textFieldCodigoDisciplina.getText().trim().toUpperCase())) {
			JOptionPane.showMessageDialog(null, "Código da disciplina inexistente!");
			textFieldCodigoDisciplina.requestFocus(); // Seta o foco para o componente
			// .trim() remove espaço da caixa de texto
		} else if (gerente
				.verificaSeExisteIdentificacaoTurma(this.posArray,textFieldIdentificacaoDisciplina.getText().trim().toUpperCase())) {
			JOptionPane.showMessageDialog(null, "Identificação da turma já está sendo usada!");
			textFieldIdentificacaoDisciplina.requestFocus(); // Seta o foco para o componente
		} else {
			try {
				...
				gerente.editarTurma(this.turma.getCodigo(), turma);
				JOptionPane.showMessageDialog(null, "Turma cadastrada com sucesso!");
				this.dispose();

			} catch (Exception e1) {
				JOptionPane.showMessageDialog(null, "Erro ao cadastrar a turma!\n" + e1.getMessage());
			}
