...
public class ProfessorCadastraView extends JDialog {
	 ...
	 */
	public ProfessorCadastraView() {
		setDefaultCloseOperation(JDialog.DO_NOTHING_ON_CLOSE);
		setModal(true);
		setIconImage(Toolkit.getDefaultToolkit()
				.getImage(ProfessorCadastraView.class.getResource("/br/com/ufpb/poo/projeto/images/ufpb.png")));
		setTitle("Professor");
			...
			okButton.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if (professor.getNome().isEmpty()) {
						mensagem = "Por favor, digite o nome do professor!";
						textFieldNome.requestFocus(); // Seta o foco para o componente
					} else if (professor.getMatricula().isEmpty()) {
						mensagem = "Por favor, digite a matrícula do professor!";
						textFieldMatricula.requestFocus(); // Seta o foco para o componente
					} else if (gerente.verificaSeExisteProfessor(professor.getMatricula())) {
						mensagem = "A matrícula do professor já está sendo usada!\n";
						// buscar o professor para ter uma referencia
						try {
							mensagem = mensagem + "Professor :"
									+ gerente.buscaProfessor(professor.getMatricula()).getNome();
						} catch (Exception e1) {
							mensagem = "Erro ao buscar aluno!\n" + e1.getMessage();
						}
						textFieldMatricula.requestFocus(); // Seta o foco para o componente
					} else {
						gerente.cadastrarProfessor(professor);
